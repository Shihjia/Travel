...
- 狀態：✅ 完成（2026-09-04 Claude Code 實作完畢，待驗收）（**2026-09-04已修訂一次：Day2交通方式已從「尚未定案」改為「已定案採計程車」，本版已同步更新下方資料結構，請以本版為準**）
- 建立日期：2026-09-04
- 提出者：Claude（依使用者需求整理）
...
costs: {
  ...
  taxiSubtotal: 19850,   // Day2，日幣

  transportSubtotalConfirmed: 85850,   // charterSubtotal + taxiSubtotal，皆為日幣，已定案總交通費
...
3. **計程車費用區塊**：列出 `taxis` 陣列（目前只有Day2一筆，已定案），顯示天數／各分段車資明細（`detail`）／總金額，並顯示 `note`（分3次叫車、無兒童座椅）。區塊底部顯示「計程車小計：JPY 19,850」。
4. 包車＋計程車兩區塊下方可額外顯示「交通費用合計（已定案）：JPY 85,850」（`transportSubtotalConfirmed`，因兩者皆為日幣，這裡可以合計）。
...
- [ ] 計程車區塊顯示 Day2 一筆，金額¥19,850，含分段車資明細與「需分3次叫車、無兒童座椅」的說明
- [ ] 交通費用合計（JPY 85,850）有顯示
...
- 做了什麼：
  ...
  4. 計程車區塊：Day2 一張卡，含三段車資明細與「需分3次叫車、無兒童座椅」提醒（加粗＋⚠️）；底部「計程車小計 JPY 19,850」。
  5. 交通合計：獨立一張 `.card.card--outline`，顯示「交通費用合計（已定案）JPY 85,850」，並註明「同為日幣故可合計」。
...
- 待確認／已知問題：
  1. `index.html`「🎒 行前準備」下方的 `section-note` 仍寫「出發前對照打勾，不漏東漏西；勾選狀態會記在這台裝置的瀏覽器裡」，這句現在只描述得到打包清單、描述不到費用試算。規格沒有要求改這句，所以我沒有動；若要改成同時涵蓋兩張卡，跟我說一句就好。
  2. 沖繩世界的童票估算（約NT$180-200）在頁面上已用黃框＋兩顆警示膠囊標明是估算值，但畢竟是反推值，實際金額仍以 Klook 下單頁面為準——這是規格本身的性質，不是實作問題，這裡只是再提醒一次。
