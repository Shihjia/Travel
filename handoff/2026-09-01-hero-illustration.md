...
| # | 驗收標準 | 結果 | 實測證據 |
|---|---|---|---|
| 1 | 總覽頁 hero 顯示插圖、白字清楚可讀 | ✅ | 桌機與手機截圖確認，四段文字都清楚 |
| 2 | 首頁行程卡封面同一張圖、日期標籤清楚 | ✅ | `.trip-card__cover` 的 `background-image` 含 `hero-okinawa`；白底膠囊在圖上清楚 |
| 3 | **day 頁面與其他頁面維持原漸層** | ✅ | 逐頁量測 `getComputedStyle(.hero).backgroundImage`：總覽頁 **true**；day1／packing／首頁 `.portal-hero` 皆 **false** |
| 4 | 圖片 ≤300KB、寬 ≤2400px | ✅ | 134KB／1584×672 |
| 5 | 手機裁切後主要元素仍在畫面內、不變形 | ✅ | 375px 截圖確認鯨鯊、小船、沙灘、椰子樹、紅瓦屋、飛機都在 |
| 6 | Console 無錯誤 | ✅ | 無 |

### 文字可讀性實測：你的判斷是對的，我的原數值不足 ✅

**這次要明確說一句：你推翻我給的遮罩數值是正確的，而且方法比我好。**

我當初是把四種方案套上去截圖用眼睛比較，選了看起來「文字清楚、插圖不被壓暗」的一組。你則是把底圖畫進 canvas、依 `cover` 的縮放與位移換算出每段文字底下對應的圖片區域、取**最亮的 20% 像素**（最難讀的地方）再套遮罩 alpha 算 WCAG 對比度 —— 這是可量化、可重現的作法。目測確實很容易高估白字在中亮度背景上的可讀性，尤其是「返回連結」「2026 年度行程」這兩段小字，我的數值下只有 2.82／2.61，遠低於 4.5 的門檻。

改後桌機四段 4.54-6.86、手機四段 4.29-5.08，**8 項全部達標**。

**關於你在待確認第 1 點問的「視覺上能不能接受」—— 我實際截圖看過了，兩個尺寸都保留現在的數值，不用折衷：**

- **桌機**：左側加深到 0.70 之後文字非常清楚，而遮罩在 80% 之後歸零，**右側的紅瓦屋、風獅爺、椰子樹、沙灘完全沒被壓暗**，插圖該亮的地方都還是亮的。視覺上比我原本那版更好。
- **手機**：整片約 0.65 確實比原圖暗，但看起來像是刻意的「深海／傍晚」色調，不是糊掉。鯨鯊、小船、沙灘、椰子樹、紅瓦屋、飛機全部清楚可辨。**不需要降到 0.55 去換取插圖亮度而犧牲一般文字的對比度。**

### 需要調整的地方：無

- **CSS 圖片路徑改成 `../2026-okinawa/images/...`** —— 正確，是我寫規格時沒考慮到樣式會放在共用 CSS 檔。
- **沒有重新壓縮圖片** —— 正確，檔案本來就符合規格，再壓只會損失畫質。
- **`.trip-card__cover` 保留漸層當載入前底色** —— 好做法，避免圖片載入前出現白框。

**給規格端(Claude)的提醒**：日後寫視覺相關規格時，遮罩／對比度這類數值不要只憑截圖目測就寫成「已實測、不用再調」，應該保留讓工程端用量化方式驗證的空間，或直接要求用 WCAG 對比度驗證。
